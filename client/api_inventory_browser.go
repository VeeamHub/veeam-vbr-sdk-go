...
type ApiTestVmwareHostObjectRequest struct {
	ctx context.Context
	ApiService *InventoryBrowserApiService
	xApiVersion *string
	name string
	vmwareObjectModel *VmwareObjectModel
}

// Version and revision of the client REST API. Must be in the following format&amp;#58; &#x60;&lt;version&gt;-&lt;revision&gt;&#x60;.
func (r ApiTestVmwareHostObjectRequest) XApiVersion(xApiVersion string) ApiTestVmwareHostObjectRequest {
	r.xApiVersion = &xApiVersion
	return r
}

func (r ApiTestVmwareHostObjectRequest) VmwareObjectModel(vmwareObjectModel VmwareObjectModel) ApiTestVmwareHostObjectRequest {
	r.vmwareObjectModel = &vmwareObjectModel
	return r
}

func (r ApiTestVmwareHostObjectRequest) Execute() (map[string]interface{}, *http.Response, error) {
	return r.ApiService.TestVmwareHostObjectExecute(r)
}

/*
TestVmwareHostObject Get VMware vSphere Server Objects

DEBUG ONLY! REMOVE BEFORE RELEASE

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param name Name of the VMware vSphere server.
 @return ApiTestVmwareHostObjectRequest
*/
func (a *InventoryBrowserApiService) TestVmwareHostObject(ctx context.Context, name string) ApiTestVmwareHostObjectRequest {
	return ApiTestVmwareHostObjectRequest{
		ApiService: a,
		ctx: ctx,
		name: name,
...
func (a *InventoryBrowserApiService) TestVmwareHostObjectExecute(r ApiTestVmwareHostObjectRequest) (map[string]interface{}, *http.Response, error) {
	var (
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "InventoryBrowserApiService.TestVmwareHostObject")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}
	...
	if r.vmwareObjectModel == nil {
		return localVarReturnValue, nil, reportError("vmwareObjectModel is required and must be specified")
	}
	...
	localVarHeaderParams["x-api-version"] = parameterToString(*r.xApiVersion, "")
	// body params
	localVarPostBody = r.vmwareObjectModel
	if r.ctx != nil {
		// API Key Authentication
		if auth, ok := r.ctx.Value(ContextAPIKeys).(map[string]APIKey); ok {
			if apiKey, ok := auth["Bearer"]; ok {
